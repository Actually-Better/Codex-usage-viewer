...
- Seed upgrade baselines only while signed in, expire stale observations, serialize initialization, sign-out suppression, accepted reads, and notification opt-out, discard pre-logout refresh data and rebaseline after in-place authentication, restore visuals only from limits in the last accepted read, evaluate alerts only from accepted refresh reads, and clear visible capacity notifications on recovery or opt-out.
