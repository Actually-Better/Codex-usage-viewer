...
- Seed upgrade baselines only from recent signed-in snapshots, expire stale observations and their persistent alerts, serialize initialization, sign-out suppression, accepted reads, and notification opt-out, discard pre-logout refresh data and rebaseline after in-place authentication, restore visuals only from limits in the last accepted read, evaluate alerts only from accepted refresh reads, and clear visible capacity notifications on recovery or opt-out.
