...
- A manual refresh reuses Analytics only when you are already viewing it. From any other page it opens a temporary Analytics tab in the background, including when Analytics exists only in the background. The temporary tab is closed after reading without changing focus; if sign-in is required, it remains open in the background so you can select it and sign in through ChatGPT.
