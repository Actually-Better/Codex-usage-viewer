...
For a manual refresh, the extension reuses Codex Analytics only when it is the currently active page. From every other page it creates a normal visible temporary Analytics tab—even if another Analytics tab exists in the background—waits for its UI to render completely, and closes only the tab it created. It returns focus to the page you were using if the temporary tab is still active; if you switch elsewhere during collection, it leaves your newer selection alone. This is more reliable than embedding Analytics in a hidden frame and does not require the user to open or keep Analytics visible.
...
- A manual refresh reuses Analytics only when you are already viewing it. From any other page it opens a visible temporary Analytics tab, including when Analytics exists only in the background. The temporary tab is closed after reading and focus returns to the previous page only if you have not switched elsewhere; if sign-in is required, it remains open so you can sign in through ChatGPT.
- The 15-minute periodic check uses the same recovery path: unless Analytics is the active page, it creates a real visible temporary tab for every collection. It closes the tab after reading and restores the previously active tab. If that scheduled tab requires sign-in, it is also closed and the popup asks you to run a manual refresh. The extension also repairs the periodic alarm whenever its background worker starts.
